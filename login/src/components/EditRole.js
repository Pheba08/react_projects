...
const EditRole = () => {
  ...
  useEffect(() => {
    if (location.state?.role) {
      setRole(location.state.role.name);
      setDescription(location.state.role.description);
    }
  }, [location.state]);
  ...
  const handleUpdate = () => {
    if (role.trim() && description.trim()) {
      ...
      navigate("/rolemanagement", { state: { updatedRole } });
    }
  };

  return (
    <div className="min-h-screen bg-[#E9F3FC] overflow-hidden">
      <NavMaster heading="Edit Role" showIcons={false} />
...
//   const role = state?.role || { id: null, name: "", description: "" };
...
//   const handleSave = () => {
//     navigate("/rolemanagement", { state: { updatedRole: editedRole } });
//   };
