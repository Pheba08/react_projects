...
const EditGrade = () => {
  ...
  useEffect(() => {
    if (location.state?.grade) {
      setGrade(location.state.grade.name);
      setDescription(location.state.grade.description);
    }
  }, [location.state]);

  const handleUpdate = () => {
    if (grade.trim() && description.trim()) {
      const updatedGrade = {
        id: location.state.grade.id,
        name: grade,
        description: description,
      };
      navigate("/grade", { state: { updatedGrade } });
    }
  };

  return (
    <div className="min-h-screen bg-[#E9F3FC] overflow-hidden">
      <NavMaster heading="Edit Grade" showIcons={false} />
...
//   const grade = state?.grade || { id: null, name: "", description: "" };
...
//   const handleSave = () => {
//     navigate("/grade", { state: { updatedGrade: editedGrade } });
//   };
