...
const EditGender = () => {
  ...
  useEffect(() => {
    if (location.state?.gender) {
      setGender(location.state.gender.name);
      setDescription(location.state.gender.description);
    }
  }, [location.state]);

  const handleUpdate = () => {
    if (gender.trim() && description.trim()) {
      const updatedGender = {
        id: location.state.gender.id,
        name: gender,
        description: description,
      };
      navigate("/gender", { state: { updatedGender } });
    }
  };

  return (
    <div className="min-h-screen bg-[#E9F3FC] overflow-hidden">
      <NavMaster heading="Edit Gender" showIcons={false} />
