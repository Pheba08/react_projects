...
const EditSubject = () => {
  ...
  useEffect(() => {
    if (location.state?.subject) {
      ...
      setDescription(location.state.subject.description);
    }
  }, [location.state]);
  ...
  const handleUpdate = () => {
    if (subject.trim() && description.trim()) {
      ...
      navigate("/subject", { state: { updatedSubject } });
    }
  };

  return (
    <div className="min-h-screen bg-[#E9F3FC] overflow-hidden">
      <NavMaster heading="Edit Subject" showIcons={false} />
...
//   const subject = state?.subject || { id: null, name: "", description: "" };
...
//   const handleSave = () => {
//     navigate("/subject", { state: { updatedSubject: editedSubject } });
//   };
