...
const EditTopic = () => {
  ...
  useEffect(() => {
    if (location.state?.topic) {
      setTopic(location.state.topic.name);
  ...
  const handleUpdate = () => {
    if (topic.trim() && description.trim()) {
      ...
      navigate("/topic", { state: { updatedTopic } });
    }
  };

  return (
    <div className="min-h-screen bg-[#E9F3FC] overflow-hidden">
      <NavMaster heading="Edit Topic" showIcons={false} />
...
//   const topic = state?.topic || { id: null, name: "", description: "" };
...
//   const handleSave = () => {
//     navigate("/topic", { state: { updatedTopic: editedTopic } });
//   };
